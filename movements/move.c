...
/*   Updated: 2023/06/13 12:00:50 by aankote          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int check_move(t_data *data, int x, int y)
{
    ...
    if(data->map[(int)(y) / CARE][(int)(x) / CARE] == '1')
        return(0);
    if(data->map[(int)(y) / CARE][(x) / CARE] == '1')
        return (0);
    if (data->map[(y ) / CARE][(int)(x) / CARE] == '1')
        return (0);
    return (1);
}
...
int move_down(t_data *data)
{
    ...
    data->player.movestep = data->player.walk_dir * MOVE_SPEED;
	x = data->player.px_pos + cosin * data->player.movestep;
	y = ceil(data->player.py_pos  + sino * data->player.movestep);
    if(check_move(data, x, y))
        update_walk(data);
    return (0);
}
